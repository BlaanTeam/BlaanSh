...
int	check_option(char *cmd, char *arg)
{
	if (arg[0] == '-')
	{
		if (arg[1] == '\0')
			...
			return (shell_error(cmd, arg, "invalid option", 2), -1);
	}
	return (0);
}
...
static void	change_dir(char *next_path, t_venv **venv)
{
	...
	if (chdir(next_path) == -1)
		shell_error("cd", next_path, strerror(errno), 1);
	else
	{
		ft_getcwd();
		if ((ft_memcmp(next_path, ".", 2) == 0 || \
		ft_memcmp(next_path, "..", 3) == 0) && errno == ENOENT)
			return (shell_error("cd", "error retrieving current directory: \
...
void	cd(char **av, t_venv **venv)
{
	...
	if (av[0] == NULL)
	{
		var = getvenv("HOME");
		if (!var)
			return (shell_error("cd", "HOME not set", NULL, 1));
	...
	if (opt == 1)
	{
		var = getvenv("OLDPWD");
		if (!var)
			return (shell_error("cd", "OLDPWD not set", NULL, 1));
		return (change_dir(var, venv));
	}
	else if (opt == 0)
